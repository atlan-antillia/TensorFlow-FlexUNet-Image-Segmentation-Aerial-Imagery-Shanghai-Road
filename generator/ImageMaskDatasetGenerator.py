...
class ImageMaskDatasetGenerator:
  ...
  def generate(self, images_dir, masks_dir, output_images_dir, output_masks_dir):
    ...
    if num_image_files != num_mask_files:
      raise Exception("Unmatched image and mask files")
    
    self.generate_files(image_files, output_images_dir, mask=False)
    self.generate_files(mask_files,  output_masks_dir,  mask=True)

  def generate_files(self, image_files, output_images_dir, mask=False):
    index = 10000
    for image_file in image_files:
      index += 1
      basename = os.path.basename(image_file)
      image = cv2.imread(image_file)
      if basename.startswith("invert-"):
        image = cv2.bitwise_not(image)
      image = cv2.resize(image, self.RESIZE)
      if mask and self.mask_colorize:
      ...
      if self.augmentation:
        self.augment(image, filename, output_images_dir, border=(0, 0, 0), mask=mask)
  ...
  def colorize_mask(self, mask):
    ...
    colorized[np.equal(mask, 255)] = self.color
    return colorized
...
if __name__ == "__main__":
  images_dir = "./Shanghai-Road/img/"
  masks_dir  = "./Shanghai-Road/mask"

  output_dir = "./Aug-Shanghai-Road-master"
  ...
  generator =  ImageMaskDatasetGenerator(size=512, augmentation=True)
